	...
	switch (type)
	{
		...
		case EFFECT_BLOOM_RADIAL:
			...
			for (i = 0; i <= 100; i += 10)
			{
				...
				glTexCoord2f(1, 0);
				glVertex2i(i + render_width, i + render_height);
			}
			glEnd();
			break;
...
void RenderPrint(int x, int y, int font, GLrgba color, const char *fmt, ...)
{
	...
	glListBase(fonts[font % FONT_COUNT].font->DisplayBase - 32);
	glColor3fv(&color.red);
	glRasterPos2i(x, y);
...
void RenderInit(void)
{
	...
	for (uint32_t i = 0; i < FONT_COUNT; i++)
		fonts[i].font = hwnd->SetupGLFont(fonts[i].name, FONT_SIZE, 32, 95);
	hwnd->glBegin();
	letterbox = false;
