...
float hue_list[] = {0.04F, 0.07F, 0.1F, 0.5F, 0.6f}; //Yellows and blues - good for lights
GLrgba bloom_color;
long last_update;
...
GLrgba get_light_color(float sat, float lum)
{
	int index;
...
bool claimed(int x, int y, int width, int depth)
{
	int xx, yy;
...
plot make_plot(int x, int z, int width, int depth)
{
...
int build_light_strip(int x1, int z1, int direction)
{
	CDeco* d;
	GLrgba color;
	int x2, z2;
...
void do_reset(void)
{
	...
	north_street = WORLD_EDGE;
	south_street = WORLD_SIZE - WORLD_EDGE;
	...
	broadway_done = false;
	west_street = WORLD_EDGE;
	east_street = WORLD_SIZE - WORLD_EDGE;
